...
# Handles all recommendation algorithms and feed generation

require "json"
require "time"

module RecommendationEngine
  # Main feed types available
  enum FeedType
    Hot
    New
    Top
  ...
  def self.get_feed(user_id : Int64? = nil, feed_type : FeedType = FeedType::Hot, 
                    limit : Int32 = 50, offset : Int32 = 0) : Array(Hash(String, JSON::Type))
    case feed_type
    when FeedType::Hot
  ...
  def self.get_hot_feed(limit : Int32 = 50, offset : Int32 = 0) : Array(Hash(String, JSON::Type))
    result = POOL.exec(
    ...
    result.rows.map do |row|
      {
        ...
        "upvotes"       => row[7]?.try &.to_i || 0,
        "downvotes"     => row[8]?.try &.to_i || 0,
        "feed_type"     => "hot"
      }
    end
  end
  ...
  def self.get_new_feed(limit : Int32 = 50, offset : Int32 = 0) : Array(Hash(String, JSON::Type))
    result = POOL.exec(
      "SELECT id, title, url, source, score, comment_count, created_at, upvotes, downvotes
       FROM posts
       WHERE is_user_post = false
  ...
  def self.get_top_feed(limit : Int32 = 50, offset : Int32 = 0) : Array(Hash(String, JSON::Type))
    result = POOL.exec(
      "SELECT id, title, url, source, score, comment_count, created_at, upvotes, downvotes
       FROM posts
       WHERE is_user_post = false
  ...
  def self.get_personalized_feed(user_id : Int64, limit : Int32 = 50, offset : Int32 = 0) : Array(Hash(String, JSON::Type))
    ...
    scored_posts.first(limit).map do |_, post|
      post.merge({"feed_type" => "personalized"})
    end
  end

  # Calculate personalized score for a post
  def self.calculate_personalized_score(post : Hash(String, JSON::Type),
                                        source_scores : Hash(String, Float64),
                                        tag_scores : Hash(String, Float64)) : Float64
    # Base score from community engagement
    base_score = Math.log([post["score"].to_f, 1].max) / 10.0
    
    # Source preference
    source = post["source"].to_s
    source_boost = source_scores[source]? || 0.0
    normalized_source = Math.tanh(source_boost / 10.0)  # Normalize to 0-1
    
    # Tag preference
    tags = AlgoDB.extract_tags_from_text(post["title"].to_s)
    tag_boost = 0.0
    tags.each do |tag|
    ...
    time_boost = 1.0 / (1.0 + hours / 72.0)  # Half-life of 72 hours
  ...
  def self.get_trending_feed(limit : Int32 = 20) : Array(Hash(String, JSON::Type))
    AlgoDB.get_trending_posts(limit).map do |post|
      post.merge({"feed_type" => "trending"})
    end
  end

  # Discovery feed - new sources the user hasn't explored
  def self.get_discovery_feed(user_id : Int64, limit : Int32 = 20) : Array(Hash(String, JSON::Type))
    AlgoDB.get_discovery_posts(user_id, limit).map do |post|
      post.merge({"feed_type" => "discovery"})
    end
  end

  # Collaborative feed - what similar users like
  def self.get_collaborative_feed(user_id : Int64, limit : Int32 = 20) : Array(Hash(String, JSON::Type))
    AlgoDB.get_collaborative_recommendations(user_id, limit).map do |post|
      post.merge({"feed_type" => "collaborative"})
    end
  end

  # Mixed feed - combines multiple feed types
  def self.get_mixed_feed(user_id : Int64, limit : Int32 = 50) : Array(Hash(String, JSON::Type))
    feeds = [] of Array(Hash(String, JSON::Type))
